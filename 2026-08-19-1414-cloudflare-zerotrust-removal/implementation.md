...
- [x] 归档 frp 配置 → 独立档案 [../2026-08-19-1420-frp-config-archive/](../2026-08-19-1420-frp-config-archive/)(最初落在本目录 `frp/`,同日应用户要求 `git mv` 移出)
...
| `~/plans/2026-08-19-1414-cloudflare-zerotrust-removal/` | 新增(本档案) |
| `~/plans/2026-08-19-1420-frp-config-archive/` | 新增(frp 配置实体快照,独立成档) |
...
- frp 配置含 token,快照用 `install -m644` 落到 `/tmp` 再拷贝,绕开线上文件 0600 root 权限,agent 无需 sudo 读配置;**但本仓库实为公开仓库,快照入库前必须先脱敏**——2026-08-19 已因此泄露并清除+轮换,见 [../2026-08-19-1528-frp-token-redaction/](../2026-08-19-1528-frp-token-redaction/)。
