...
- [x] 通读源档案：`2026-08-18-sunshine-moonlight-tailnet/`（design+implementation）、`2026-08-19-headscale-custom-derp/`（design+implementation 含全部配置实体）、`2026-08-18-tailnet-exit-singbox/`（design+README+implementation 的 tailnet 部分）、`2026-08-19-frp-config-archive/`、`2026-08-19-cloudflare-zerotrust-removal/` 关联条目 + 记忆 `moonlight-sunshine-tailscale-setup`
- [x] 参照 `2026-08-21-sunshine-notebook/`、`2026-08-22-singbox-notebook/` 确定结构与约定（frontmatter + wikilink + 证据链留 plans + 机密红线）
...
- **frp 本体不搬入**：沿用 singbox-notebook 决策（frp 配置实体留在 `2026-08-19-frp-config-archive/`），本域只在架构总览与深坑清单交叉引用
- **Notebook git 状态与既有档案的说法差异**：`2026-08-22-singbox-notebook/design.md` 写"~/Notebook 不入 git"，该决策已于同日被用户要求推翻（初始导入 commit 9d08edc，private github.com/desmondc9/notebook，经 10809 代理 push）；本档案如实记录当前状态
