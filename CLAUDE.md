...
本仓库(`home-folder-linux-plans`)是家目录系统配置任务的设计/实施档案库,托管在 GitHub 且为 **公开仓库**(https://github.com/desmondc9/home-folder-linux-plans)。每个任务一档:`[日期]-[主题]/spec.md` + `implementation.md`,索引维护在 [README.md](README.md)。
...
- **spec.md 必须记录环境**(2026-09-12 约定,当日已全量回填):头部 `- 环境:` 行(推荐,单行可扫描)或「## 环境」章节,写明操作系统与桌面栈(如 Kubuntu 26.04 + Wayland 笔记本 yaoshi15pro / WSL2 Ubuntu 26.04 @ Win11 物理机 DESKTOP-J7NBNU4 / Bandwagon VPS / Windows 11 物理机)、涉及的机器与版本号——同一问题在不同 OS 上根因与修法不同,环境信息是档案可复用的前提。
...
**2026-08-19 事故**:frp `auth.token` 随配置快照被提交并推送到本公开仓库,发现后从全部 git 历史清除(filter-repo + 强推)并轮换。事故档案:[2026-08-19-1528-frp-token-redaction/](2026-08-19-1528-frp-token-redaction/)。规则由此而来:

1. **任何 token / 密码 / 私钥 / API key / 预授权密钥(preauth key)/ 会话凭据,一律不得写入任何被 git 跟踪的文件**——包括配置实体快照、日志片段、systemd 单元(`ExecStart` 可能内嵌 token)、spec.md / implementation.md 正文。没有"这是私人仓库所以可以"的例外——入库前先确认仓库可见性。
