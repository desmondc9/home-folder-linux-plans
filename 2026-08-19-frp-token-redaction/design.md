...
日期:2026-08-19 · 状态:git 侧已完成;token 轮换见 [implementation.md](implementation.md)
...
7. **轮换**(收尾必需):公开过的 token 当作已死,生成新 token 同步改 VPS `frps.toml` + 本机 `frpc.toml`,两端重启。历史清除只止血,轮换才除根——GitHub 对已强推掉的 commit 仍有缓存期,且无法排除已被人 clone/抓取。
...
- [ ] 新 token 双端生效,`frpc.service` active,`ssh -p 6000 desmond@laptop.signal-align.com` 链路可用
